...
def get_auxiliary_func(f: Foo, barrier_functions, rk) -> Foo:
    return Foo(
        ...
        lambda x: f.get_gradient(x) - rk * sum(map(lambda g: -g.get_gradient(x) / (g.get_func(x) ** 2), barrier_functions)))
...
def barrier_method(f: Foo, barrier_functions, x0, r0, c) -> float:
    ...
    while condition:
        ...
        xk = descent_with_const_step(auxiliary_func, cur_point, barrier_functions)
